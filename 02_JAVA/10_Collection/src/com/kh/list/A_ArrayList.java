 ...
 *            
 * - java.util 패키지에 컬렉션과 관련한 인터페이스와 클래스들이 포함
 ...
 * - 동기화 : 하나의 자원(데이터)에 대해 여러개의 스레드가 접근하려 할 떄 
...
//		list.add(3);
	...
	public void method2() {
	   ...
	   //     -> 내부적으로 기존에 있는 것들은 뒤로 떙기고 해당 인덱스에 값 추가
	   list.add(1, new Person("유재석", "논현동", 51));
	   list.add(3, new Person("강호동", "도곡동", 54));
	   
	   // 3. set : 해당 인덱스의 값을 변경
	   //    -> 다른 값들의 위치 변화 X
	   list.set(0, new Person("김현호", "하남시", 28));
	   ...
	   List<Person> sub = list.subList(0, 3);
	   System.out.println(sub);
	   System.out.println(list);
	   
	   // 8. addAll : 컬렉션을 통쨰로 뒤에 추가
	   list.addAll(sub);
	   System.out.println(list);
	...
	public void method3() {
	   ...
	   // 11. 오름차순 정렬 : Comparalbe 인터페이스를 구현하고 있는 요소를 가지고 비교값들을 반환하여 정렬
	   Collections.sort(list);
	   
	   // 12. 내림차순 정렬 : sort 메서드를 통해서 오름차순 정렬 후 reverse 메서드를 호출하여 내림차순으로 변경
	   // reverse 자체가 내림차순 정렬 기능이 있는 것은 아님!
