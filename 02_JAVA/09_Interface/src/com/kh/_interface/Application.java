 ...
 * - 객체가 구현해야하는 기능이 있을 때 인터페이스에 추상 메서드를 추가해,
 ...
 * - 인터페이스는 interface 키워드로 정의되어 있고, 인터페이스 내에 일반 변수나 메서드 생성 불가능!
 * 
 * extends와 implements
...
//		Vehicle v = new Vehicle(); -> 인터페이스는 객체 생성이 불가!! X
		Vehicle bus = new Bus();
	 ...
	 * 인터페이스 : 기본 설계도
	 * 
	 * 개발방법론 : 예전 - 폭포수(각 단계가 마무리 된 후 다음 단계) vs 요즘 - 애자일(모든 파트가 동일한 시작점을 가지고 개발을 진행)
	 * 
	 */
