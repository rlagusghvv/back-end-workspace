...
//		-> 원본 배열을 오름차순 정렬한 다음, 값을 반대로 새로운 배열에 대입
	...
	/*
	 * 선택정렬(selection sort) - 가장 작은 데이터를 선택해 맨 앞에 있는 데이터와 바꾸고, 그 다음 작은 데이터를 선택해 앞에서
	 * 두 번째 데이터와 바꾸는 과정을 반복 - 배열을 전부 탐색하여 최소값을 고르고 왼쪽부터 채워나가는 방식의 정렬 - 데이터 양이 적을 때
	 * 좋음! but! 100개 이상인 경우 급격한 속도 저하가 나타남.
	 */
	 ...
	 * - 필요할 때만 위치를 바꾸므로 '데이터가 거의 정렬되어 있을 떄' 효율적!
	 */
