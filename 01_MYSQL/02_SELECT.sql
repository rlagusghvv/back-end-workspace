...
/* 1. 가장 대표적인 데이터베이스 시스템
   2. 데이터를 테이블 형태로 저장하고 여러 테이블을 조합하여 비즈니스 관계(기능)를 도출하는 구조
   3. 데이터의 중복을 최소화할 수 있으며, 사용하기 편리하고, 데이터의 무결성(데이터의 일관성, 정확성 ..), 트랜잭션 처리 등
   데이터베이스 관리 시스템으로 뛰어난 성능을 보여준다.
*/
-- CRUD는 데이터베이스에서 데이터를 생성(Create), 읽기(Read), 수정(Update), 삭제(Delete)
-- 하는 네 가지 기본 작업을 의미합니다. (백엔드의 기초!!)
...
/* 1. DML(Data Manipulation Language) : 데이터 조작어
   ...
   (COMMIT : 실행, ROLLBACK : 취소) * MYSQL은 ORACLE과 달리 AUTO COMMIT이 default
*/


/*
   SELECT 컬럼, (컬럼, ...) (여러 개가 올 수 있음)
   FROM 테이블명;
...
/*
  ...
  - 산술연산 시 컬럼명이 지저분해짐.
    이때 컬럼명에 별칭을 부여해 깔끔하게 보여줄 수 있다.
    
  - 별칭 부여시 띄어쓰기 또는 특수문자가 포함되는 경우 반드시 큰따옴표(")로 감싸준다.
*/
...
-- employee에서 월급(salary)가 500만원 이하인 사원들만 조회
...
-- 실습문제 (테이블 : employee) --
-- 1. 부서코드(dept_code)가 'D1'인 사원들의 사원명 (emp_name), 급여, 부서코드만 조회
...
/*
  BETWEEN AND
  - 조건식에서 사용되는 구문
  - 몇 이상 몇 이하인 범위에 대한 조건을 제시할 때 사용되는 연산자! (특정 범위 지정 가능!)
...
FROM employee
WHERE email LIKE '___\_%'; -- \문자 : 문자를 문자 그대로 인식!
...
-- 부서배치를 아직 받지 않고 보너스 받은 사원들의 이름, 보너스, 부서코드 조회
...
/*
   ...
   ORDER BY
   - SELECT문 가장 마지막 줄에 작성 뿐만 아니라 실행순서 또한 마지막에 실행alter
...
FROM employee
ORDER BY bonus DESC; -- 보너스 값을 내림차순 정렬 (NULL은 가장 값이 작음!)

SELECT emp_name, bonus
FROM employee
WHERE bonus IS NOT NULL -- 먼저 보너스 값이 NULL 인 경우는 제외
-- ORDER BY bonus -- NULL이 아닌 보너스 값을 오름차순 정렬
ORDER BY 1 DESC; -- 숫자를 넣으면 SELECT의 컬럼 순서에 맞는 컬럼으로 치환
...
-- 4. 입사일이 '1995-01-01' 이상이고 부서배치를 받은 사원들의 사번, 사원명, 입사일(hire_date), 부서코드 조회 (정렬은 먼저 입사한 기준)
